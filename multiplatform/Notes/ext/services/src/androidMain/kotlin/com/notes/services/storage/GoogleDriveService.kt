...
class GoogleDriveService : AbstractStorageService() {
    ...
    private fun getDataFileName(document: Document): String =
        if (document.isFile) {
            document.file?.name ?: document.name
        } else {
            "${document.name}.json"
        }

    private fun isNoteDataFile(name: String): Boolean = name.endsWith(".json")

    private fun getMimetype(ext: String) = MimeTypeMap.getSingleton().getMimeTypeFromExtension(ext)
            ...
            if (oldFile == null) {
                val file =
                    File().apply {
                        ...
                        if (document.isFile) {
                            val ext =
                                document.file?.extension ?: throw IllegalStateException(
                                    "GoogleDriveService::store() File extension can't be empty",
                                )
                            val mimetype =
                                getMimetype(ext) ?: throw IllegalStateException(
    ...
    private fun initGoogleDrive(token: String) {
        ...
        val credentials =
            object : GoogleCredentials(builder) {
                ...
                override fun refreshAccessToken(): AccessToken = AccessToken("", null)
            }
