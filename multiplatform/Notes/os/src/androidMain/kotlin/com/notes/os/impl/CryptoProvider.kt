...
class CryptoProvider : CryptoOperations() {
    ...
    override suspend fun encrypt(message: String): String = protectKeyStore.withLock { provider.encrypt(message).message }

    // NOTE: This call is not thread safe because KeyStore is not thread safe
    // That's why we use mutex
    override suspend fun decrypt(message: String): String = protectKeyStore.withLock { provider.decrypt(message).message }

    suspend fun testOnly_genDerivedKey(
        password: String,
        uid: String,
    ) = genDerivedKeyAndSave(password, uid)
