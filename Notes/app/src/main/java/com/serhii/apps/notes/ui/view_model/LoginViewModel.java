        ...
        if (newValue != null) {
            switch (newValue.getAuthType()) {
                ...
                case AUTH_PASSWORD_LOGIN:
                    // Proceed with Login
                    getAuthorizeService().onPasswordLogin(newValue);
                    break;
                case AUTH_REGISTRATION:
                    // Proceed with User's registration
                    getAuthorizeService().onRegistration(newValue);
                    break;
                case AUTH_BIOMETRIC_LOGIN:
                    // Proceed with Login
                    getAuthorizeService().onBiometricLogin();
                    break;
            }
        }
    }

    public IAuthorizeService getAuthorizeService() { return EventService.INSTANCE; }
