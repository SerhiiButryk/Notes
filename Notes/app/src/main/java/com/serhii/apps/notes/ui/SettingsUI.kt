    ...
    Scaffold(
        ...
        if (dialogState.canOpen()) {
            if (dialogState is SettingsViewModel.ListDialogUIState) {
                ...
                TextInputDialog(dialogState = dialogState, viewModel)
            }
        }
    }
}
...
@Composable
fun TextInputDialog(dialogState: DialogUIState, viewModel: SettingsViewModel) {

    val inputFocus = remember { FocusRequester() }
    val focusModifier = Modifier.focusRequester(inputFocus)
        ...
        )
        {
            ...
            LazyColumn(
                ...
                item {
                    ...
                    PasswordFieldUI(
                        label = stringResource(state.firstLabel),
                        hint = stringResource(state.firstHint),
                        initValue = { state.inputFirst },
                    ...
                    if (dialogState.hasSecondInput) {
                        PasswordFieldUI(
                            label = stringResource(state.secondLabel),
                            hint = stringResource(state.secondHint),
                            initValue = { state.inputSecond })
                        { newText ->
    ...
    viewModel.requestKeyboard(inputFocus)
}
